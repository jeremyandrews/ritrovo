...
| D6 | Form API: edit form, multi-step submission, profile form, subscription form | kernel, plugin | **part done (A6)** | The edit form and the profile form are usable: 36.1, 36.7. The multi-step submission is 36.4 and the subscription form is 37.3. The kernel's own Form API is reachable from no route in any of them (`G-FORM-TAPS-UNREACHABLE`); every form Ritrovo serves is a `tap_api` route with a `_token` in the body. | A6 |
