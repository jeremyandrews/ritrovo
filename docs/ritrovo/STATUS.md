...
| # | The brief promises | Where | Status | Evidence | Closes in |
| :- | :- | :- | :- | :- | :- |
| 35.1 | Registration, login, sessions, and three test users | config | **Ritrovo must build it** | Login and `/user/register` work (kernel, `crates/kernel/src/routes/auth.rs:1621-1639`); `/admin/people` lists only the administrator. The three editorial users do not exist. | A5 |
| 35.2 | Role-based permissions with plugin Grant and Deny | config, plugin | **blocked on the kernel** | Roles `viewer`, `editor`, `publisher` hold kernel permissions only; no role holds a Ritrovo permission (`G-PERM-TAP-NOT-DISPATCHED`, `G-PERM-GRID-SAVE-REVOKES-PLUGIN-GRANTS`). | A5 |
| 35.3 | Incoming, Curated, Live workflow with enforced transitions | config, kernel | **blocked on the kernel** | All 5,656 items on Live; stages "in use by nothing"; no way to move an item (`G-NO-ITEM-STAGE-TRANSITION`, `G-DEFAULT-STAGE-IGNORED-ON-CREATE`). | A5 |
| 35.4 | Revision history with revert, five scenarios | kernel | **blocked on the kernel** | History 500 after one edit (`G-REVISION-HISTORY-500`); no compare (`G-REVISION-NO-COMPARE`); imports create no revisions. | A5 |
| 35.5 | Admin content list with filters and bulk actions | kernel | **done** | `/admin/content` filters by type and status and offers bulk publish, unpublish and delete, with per-row Edit, Revisions, Delete. Administrator only, as the story specifies. | A5 |
...
| 39.2 | Batch operations with progress | kernel | **blocked on the kernel** | `G-BATCH-NO-EXECUTOR`; no stage bulk action (`G-NO-ITEM-STAGE-TRANSITION`). | A5 |
...
| D25 | Batch: bulk publish Curated to Live, bulk re-import, progress | kernel | **blocked on the kernel** | 39.2. | A5 |
...
| P18 | `ritrovo_access` `tap_perm`: declare the editorial permissions | plugin | **blocked on the kernel** | `lib.rs:12`; never dispatched (`G-PERM-TAP-NOT-DISPATCHED`). | A5 |
